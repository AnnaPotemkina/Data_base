...
class Addition
{
    ...
    public function addition(array $data): bool{
       ...
       /* $size_ids = $this->database->getConnection()->query(
        ...
        $width = $product_parametrs[0];
        $height = $product_parametrs[1];
        $length = $product_parametrs[2];
        foreach ($size_ids as $Size_id){
            if (((int) $width == $Size_id["Product_width"]) and ((int) $height == $Size_id["Product_height"]) and ((int) $length == $Size_id["Product_length"])){
...
*/
        ...
        $statement = $this->database->getConnection()->prepare(
            "INSERT INTO Product (Type_id, Code , Price , Available_number, Type_furniture)
            VALUES (:type_id, :code, :price, :available_number, :type_furniture)"
        );
        $statement->execute([
            "type_id" => $required_id,
            "code" => $data["code"],
